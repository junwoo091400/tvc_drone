...
public:
    ...
    ExtendedKalmanFilterNode(ros::NodeHandle &nh) : ekf() {
        // init publishers and subscribers
        initTopics(nh);

        double pos_std, orientation_std, vel_std, ang_vel_std, optitrack_pos_std, optitrack_orientation_std;
        if (nh.getParam("predict_std/position", pos_std) &&
            nh.getParam("predict_std/orientation", orientation_std) &&
            nh.getParam("predict_std/vel", vel_std) &&
            nh.getParam("predict_std/angular_vel", ang_vel_std) &&
            ...
            predict_std << pos_std, pos_std, pos_std,
                    orientation_std, orientation_std, orientation_std, orientation_std,
                    vel_std, vel_std, vel_std,
                    ang_vel_std, ang_vel_std, ang_vel_std;
            ...
            update_std << optitrack_pos_std, optitrack_pos_std, optitrack_pos_std,
                    optitrack_orientation_std, optitrack_orientation_std, optitrack_orientation_std, optitrack_orientation_std;
    ...
    void initTopics(ros::NodeHandle &nh) {
        ...
        optitrack_sub = nh.subscribe("/optitrack_client/car/pose", 1,
                                     &ExtendedKalmanFilterNode::optitrackCallback,
                                     this);
...
int main(int argc, char **argv) {
    ...
    ros::NodeHandle nh("optitrack_ekf");

    ExtendedKalmanFilterNode ekf_node(nh);

    // Thread to compute kalman. Duration defines interval time in seconds
    ros::Timer control_thread = nh.createTimer(ros::Duration(ekf_node.period), [&](const ros::TimerEvent &) {
        ekf_node.kalmanStep();
    });
