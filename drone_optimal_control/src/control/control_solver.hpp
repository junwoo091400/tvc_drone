...
template <typename Problem,
          typename QPSolver =
              boxADMM<Problem::VAR_SIZE, Problem::NUM_EQ + Problem::NUM_INEQ, typename Problem::scalar_t,
                      Problem::MATRIXFMT, linear_solver_traits<DroneControlOCP::MATRIXFMT>::default_solver>>
class ControlSolver : public SQPBase<ControlSolver<Problem, QPSolver>, Problem, QPSolver>
{
public:
  using Base = SQPBase<ControlSolver<Problem, QPSolver>, Problem, QPSolver>;
  ...
  scalar_t step_size_selection_impl(const Ref<const nlp_variable_t>& p) noexcept
  {
    ...
    for (int i = 1; i < this->m_settings.line_search_max_iter; i++)
    {
      ...
      // std::cout << "phi before: " << phi_l1 << " after: " << phi_l1_step <<  " required diff: " << alpha *
      // this->m_settings.eta * Dp_phi_l1 << "\n";
