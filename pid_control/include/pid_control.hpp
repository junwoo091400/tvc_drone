...
public:
  /**
   * @brief Container to manage the a cascade of PID controllers (where each stage itself consists of multiple
   * controllers)
   *
   */
   ...
   * @param u_feed_forward Reference to variable that carries the feedforward term of a higher level controller (e.g.
   * MPC), Note: Usage must be explicitely specified when calling "updateControlLaw()"
   */
  PidController(RocketProps& rocket_props, std::shared_ptr<PidController::ControlCascades>& cc,
   ...
   */
  static bool loadControllersFromFile(std::string filename, std::shared_ptr<PidController::ControlCascades>& cc,
                                      std::vector<std::string> cascade_names, std::vector<std::string> dimension_names)
  {
    YAML::Node node;
    try
    {
    ...
    for (size_t cascade_iter = 0; cascade_iter < cc->controllers.size(); cascade_iter++)
    {
      ...
      for (size_t dimension_iter = 0; dimension_iter < dimension_names.size(); dimension_iter++)
      {
        try
        {
          ...
          PID ctrl(node[cascade_names.at(cascade_iter)][dimension_names.at(dimension_iter)]["gains"][0].as<double>(),
        ...
        {
          std::cerr << "Error loading element '" << cascade_names.at(cascade_iter) << " - "
                    << dimension_names.at(dimension_iter)
                    << "'. Can not find 3-element-double-list for key 'gains' and 2-element-double-list for key "
                       "'limits'.\nError Message:\n"
                    << e.what() << std::endl;
          return false;
        }
    ...
    {
      std::cerr << "Error loading 'additional_limits'. Can not find double value for either 'alpha' or 'beta' under "
                   "this key.\nError Message:\n"
                << e.what() << std::endl;
      return false;
    }
  ...
  /**
   * @brief Implementation of a 4-stage cascaded controller (see figure 5.3 of
   * https://drive.google.com/file/d/1psKMbYIDg3n1MyOD7myFiBktjy46THxa/view?usp=sharing)
   *
   ...
   * @param use_u_feed_forward [optional, default=false] Use feedforward control input from higher level controller
   * (e.g. MPC), otherwise: Use gravity compensation as feedforward term
   */
  void updateControlLaw(RocketState current, RocketState set_point, double time_s, geometry_msgs::Vector3* angle,
                        bool use_u_feed_forward = false)
  {
    if (cc->dt < time_s - prev_time)
      ROS_WARN_STREAM("It has been " << time_s - prev_time << "s since the last update-call. Target is " << cc->dt
                                     << "s.");
    else
      ROS_DEBUG_STREAM("It has been " << time_s - prev_time << "s since the last update-call. Target is " << cc->dt
                                      << "s.");
    prev_time = time_s;
...
private:
  ...
  /**
   * @brief Given 2 angles (old and new), gets representation for new angle that is the closest to the old one (2-pi "="
   * 0)
   *
  ...
  void updateAnglesFromRocketState(RocketState s)
  {
    Eigen::Matrix<double, 3, 3> R =
        Eigen::Quaternion<double>(s.orientation.w, s.orientation.x, s.orientation.y, s.orientation.z)
            .toRotationMatrix();
  ...
  void convertAccelerationToAngle(double mass, bool use_mass_feed_forward)
  {
    ...
    if (!use_mass_feed_forward)
      tz += mass * G;
    cc->outputs.at(1).at(0) = std::max(std::min(-1 * atan2(ty, tz), cc->alpha_limit), -cc->alpha_limit);
    ty = -1 * tan(cc->outputs.at(1).at(0)) * tz;
    cc->outputs.at(1).at(1) =
        std::max(std::min(cos(cc->outputs.at(1).at(0)) * atan2(tx, tz), cc->beta_limit), -cc->beta_limit);
   ...
   *        See equations 4.5 to 4.6 of
   * https://drive.google.com/file/d/1psKMbYIDg3n1MyOD7myFiBktjy46THxa/view?usp=sharing
   * @param use_u_feed_forward If feed-forward control-output 'u_feed_forward' should be used (useful especially when
   * tracking other controller)
   */
  void controlAllocation(bool use_u_feed_forward)
  {
    ...
    double thrust = std::max(std::min(sqrt(Fz * Fz + Fy * Fy + Fx * Fx), (double)rocket_model.props.maxThrust.at(2)),
                             (double)rocket_model.props.minThrust.at(2));

    DroneGimbalControl gimbal_ctrl;
    gimbal_ctrl.inner_angle =
        theta_2 + (use_u_feed_forward ? this->u_feed_forward->inner_angle : 0);  // Rotation around body-y
    gimbal_ctrl.outer_angle =
        theta_1 + (use_u_feed_forward ? this->u_feed_forward->outer_angle : 0);  // Rotation around body-x
