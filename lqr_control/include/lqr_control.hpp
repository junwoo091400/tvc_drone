...
public:
   ...
   */
  LqrController(DroneProps<double>& drone_props, double dt, std::string QR_file_path) : dt(dt)
  {
    K.setZero();
...
private:
  ...
  /**
   * @brief Update class attribute "error" based on newly measured RocketState and desired setpoint such that Euler
  ...
  {
    error.segment(9, 3) << set_point.angular_velocity.x - current.angular_velocity.x,
        set_point.angular_velocity.y - current.angular_velocity.y,
        set_point.angular_velocity.z - current.angular_velocity.z;
    error.head(6) << set_point.position.x - current.position.x, set_point.position.y - current.position.y,
        set_point.position.z - current.position.z, set_point.velocity.x - current.velocity.x,
        set_point.velocity.y - current.velocity.y, set_point.velocity.z - current.velocity.z;
    Eigen::Matrix<double, 3, 3> R1 = Eigen::Quaternion<double>(current.orientation.w, current.orientation.x,
                                                               current.orientation.y, current.orientation.z)
                                         .toRotationMatrix();
