...
class PID{
     ...
     * @param I_gain Ki
     * @param D_gain Kd
     * @param lower_limit Output of PID is clipped, integrator state is modified such that this border is not violated by Ki * I
     * @param upper_limit Output of PID is clipped, integrator state is modified such that this border is not violated by Ki * I
     * @param error Variable containing the control error that gets updated by the main program (call-by-reference!)
     * @param error_rate Variable containing the control error rate that gets updated by the main program (call-by-reference!)
     * @param output Variable containing the controller's output that gets computed by calling the 'update()'-method (call-by-reference!)
     */
     ...
     * @param compute_error_rate_internally If error rate should be computed with the last and current error using division with dt (= first order approximation), or with the error_rate variable
