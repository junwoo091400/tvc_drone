/**
 * @file lqr_with_integrator_control.hpp
...
public:
  /**
   * @brief Construct a new LQR with integrator module by computing the relevant state feedback matrix from config in
   * file
   *
   ...
   */
  Lqr_with_integratorController(DroneProps<double>& drone_props, double dt, std::string QR_file_path) : dt(dt)
  {
    K.setZero();
  ...
  //  LEGACY CODE where K and a steady state control input where computed beforehand in Matlab and not 'dynamically' in
  //  C++ bool loadFromFile(std::string K_filename, std::string U_filename){
  //     YAML::Node node;
  //     try
  //     {
...
private:
  ...
  // Cost matrix on augmented 16D-state (12 geometrical state + 4 integrator states, see
  // https://drive.google.com/file/d/1psKMbYIDg3n1MyOD7myFiBktjy46THxa/view?usp=sharing)
  ...
  void loadQR(std::string QR_file_path)
  {
    ...
    for (size_t i = 0; i < 16; i++)
      Q(i, i) = node["Q"].as<std::vector<double>>().at(i);
    R.setZero();
    for (size_t i = 0; i < 4; i++)
      R(i, i) = node["R"].as<std::vector<double>>().at(i);
  }

  /**
   * @brief Update class attribute "state" based on newly measured RocketState and desired setpoint such that Euler
   * angles of error representation are continuous
   * @details The error is only considered for 4 selected geometric states (position + yaw angle). This error is
   * integrated in the last 4 entries of the actual state vector. See
   * https://drive.google.com/file/d/1psKMbYIDg3n1MyOD7myFiBktjy46THxa/view?usp=sharing (chapter 5.3.3 for details)
   *
   * @param current Is-state
   * @param set_point Target state
   */
  void update(RocketState current, RocketState set_point)
  {
    ...
    state.head(6) << current.position.x, current.position.y, current.position.z, current.velocity.x, current.velocity.y,
        current.velocity.z;
    Eigen::Matrix<double, 3, 3> R = Eigen::Quaternion<double>(current.orientation.w, current.orientation.x,
    ...
    Eigen::Matrix<double, 3, 3> R2 = Eigen::Quaternion<double>(set_point.orientation.w, set_point.orientation.x,
                                                               set_point.orientation.y, set_point.orientation.z)
                                         .toRotationMatrix();
    // R is the rotation matrix from is-orientation to target orientation
    R = (R2.transpose() * R);

    error << current.position.x - set_point.position.x, current.position.y - set_point.position.y,
        current.position.z - set_point.position.z, getContinuousAngle(atan2(-R(0, 1), R(0, 0)), error(3));
    state.tail(4) += error;
  }
  ...
  void discretize(Eigen::Matrix<double, NX, NX> A, Eigen::Matrix<double, NX, NX>& Phi, Eigen::Matrix<double, NX, NU> B,
                  Eigen::Matrix<double, NX, NU>& Gamma, double dt)
  {
    ...
    if (Gamma(0, 0) != Gamma(0, 0))
    {
      ...
      for (size_t i = 1; i < 1000; i++)
        Gamma += A.pow(i - 1) * pow(dt, i) * B / tgamma(i + 1);
    }
  }
};