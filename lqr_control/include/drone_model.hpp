...
public:
  ...
  void init(DroneProps<double> props_)
  {
    this->props = props_;
  }

  template <typename T, typename state>
  inline void generic_rocket_dynamics(const Eigen::Matrix<T, 12, 1> x, const Eigen::Matrix<T, 3, 1> thrust_vector,
                                      const Eigen::Matrix<T, 3, 1> body_torque,
                                      const Eigen::Matrix<T, 3, 1> disturbance_force,
                                      const Eigen::Matrix<T, 3, 1> disturbance_torque, state& xdot) const noexcept
  {
    Eigen::Quaternion<T> q = Eigen::AngleAxis<T>(x(6), Eigen::Vector3<T>::UnitX()) *
    ...
    attitude_kin << (T)cos(x(8)), (T)-sin(x(8)), (T)0, (T)sin(x(8)) * cos(x(7)), (T)cos(x(8)) * (T)cos(x(7)), 0,
    ...
    rocket_torque << thrust_vector(1) * props.total_CM, -thrust_vector(0) * props.total_CM, (T)0;
  ...
  template <typename T, typename state>
  inline void state_dynamics(const state_t<T>& x, const control_t<T>& u, const parameters_t<T>& params,
                             state& xdot) const
  {
    ...
    thrust_direction << sin(servo2), -cos(servo2) * sin(servo1), cos(servo1) * cos(servo2);
  ...
  void setParams(double thrust_scaling_val, double torque_scaling_val, double fx, double fy, double fz, double mx,
                 double my, double mz)
  {
  ...
  {
    params << (T)props.thrust_scaling, (T)props.torque_scaling, (T)props.disturbance_force.x(),
        (T)props.disturbance_force.y(), (T)props.disturbance_force.z(), (T)props.disturbance_torque.x(),
        (T)props.disturbance_torque.y(), (T)props.disturbance_torque.z();
  }
};
