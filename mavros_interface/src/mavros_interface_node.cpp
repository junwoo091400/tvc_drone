...
void RCCallback(const mavros_msgs::RCIn::ConstPtr& rc_control)
{
  ...
  if (abs(joystic_x) < 0.1)
    joystic_x = 0;
  if (abs(joystic_y) < 0.1)
    joystic_y = 0;
...
int main(int argc, char** argv)
{
  ...
  // message_filters::TimeSynchronizer<geometry_msgs::PoseStamped, geometry_msgs::TwistStamped> sync(pose_sub,
  // vel_sub,10); sync.registerCallback(boost::bind(&publishConvertedState, _1, _2));
