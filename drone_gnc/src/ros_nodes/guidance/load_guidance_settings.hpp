...
GuidanceSettings<double> loadGuidanceSettings(ros::NodeHandle &nh) {
    ...
    if (nh.getParam("mpc/min_z", settings.min_z) &&
        nh.getParam("mpc/min_dz", settings.min_dz) &&
        nh.getParam("mpc/max_dz", settings.max_dz) &&
        nh.getParam("mpc/horizontal_slack", settings.horizontal_slack) &&
        ...
        nh.getParam("target_apogee", settings.target_apogee_vec) &&
        nh.getParam("target_land", settings.target_land_vec)
            ) {
